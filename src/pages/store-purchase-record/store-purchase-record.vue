      ...
      <dropDownBox class="pt-3">
        <template v-slot:top>
          <view class="pl-4 pr-4 mb-1">
            <searchBar placeholder="输入名称" v-model="searchInput.searchKey" @clear="handleConfirm" @confirm="handleConfirm"  radius="60" />
          </view>
        </template>
        <view class="flex pl-5 pr-5 nav-item justify-between">
          <view class="mr-5 ">
            <view class="mr-2"><dropDownScrollView v-model="searchInput.goodsCode" placeholder="物料类型" :list="goodTypeList" @finally="handleConfirm" /></view>
          </view>

         

          <view >
            <dropDownScrollView placeholder="申请人" v-model="searchInput.userId" showSearch :list="userList" @finally="handleConfirm"/>
          </view>
        </view>
      </dropDownBox>
    </view>
          ...
          <view class="pt-4 pb-2 pl-4 pr-3 flex">
            <view class="flex flex-col w-100%">
              <view class="flex items-baseline mb-2">
                ...
                <text>：</text>
                <view class="ml-1"><text class="align-bottom">{{ v?.userName }}</text></view>
              </view>
<!-- 
              <view class="flex items-baseline mb-2">
                <view class="w-136rpx text-align-last-justify align-middle">
              ...
              <!-- <view class="text-right w-100% flex justify-end">
                 <view class="mr-3"> <button type="primary" size="mini" plain class="cbtn">确认领用</button></view>
                 <view> <button type="primary" size="mini" plain class="cbtn">查看详情</button></view>
              </view> -->
            </view>
          </view>
        </view>
