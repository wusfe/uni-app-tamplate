        ...
        <view class="bb1">
          <uni-forms-item label="结束图片">
            <uni-file-picker
              ...
              style="margin: 0"
              :auto-upload="false"
              @success="(e: any) => handleSuccessFile(e, 'taskEimg')"
...
import { getsysUsertaskUserId, taskinforAdd, taskinforDetail, taskinforUpdate } from '@/api'
...
const restFormInput = () => {
  const { taskType, taskUserId, taskStime, taskTitle, taskContent, taskEtime, taskEimg, taskImg } =
    detail.value
  ...
  if (taskImg) {
    console.log(taskImg, 'taskImg')

    file1.value.setValue([
      {
        url: taskImg,
      },
    ])
